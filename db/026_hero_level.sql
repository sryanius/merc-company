-- 026 · §174 영웅 — 각성한 영웅은 Lv100 까지 큰다.
--
-- 바꾸는 것:
--   · scores.top_level 제약 1~80 → 1~100 (db/001)
--   · run_mercs.level 제약 1~80 → 1~100 (db/013)
--   · run_import 의 level 접기 least(80) → least(100) (db/016 의 함수 본문을 그대로 두고 그 줄만 바꿨다)
--
...
alter table public.run_mercs drop constraint if exists run_mercs_level_check;
alter table public.run_mercs add constraint run_mercs_level_check check (level between 1 and 100);
