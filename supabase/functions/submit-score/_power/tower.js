 ...
 * 1층 1G / 100층 100G / 500층 500G, 1~500층 누적 **125,250G**.
 *
 * ★ 2 였는데 절반으로 내렸다. 실측 경제 모델에서 후반(정원 50~70) 일수지가 적자라
 *   탑을 한 번 돌면 회수가 불가능했다 — "탑까지 돌리니까 돈을 모을 수가 없다".
 *   의뢰 보상 +20% · 벤치 임금 할인과 **함께** 넣은 값이다. 셋 중 하나만 되돌리면 다시 적자다.
 */
 ...
 *   power 6.35 = 던전 10웨이브(A등급 풀세트 완주 29%)                     → 약 500층 부근
 ...
 *
 *   ★ 7.20 → 7.00 으로 다시 내렸다 (13차 세션). 층 이월 버그를 고쳤기 때문이다 —
 *     쓰러진 단원의 `carry[uid] = 0` 이 **한 층만 살고 사라져** 두 층 뒤에 만피로 복귀했다
 *     (편성에서 빠진 사람은 다음 층 carry 계산 루프에 안 잡혔다). 고치니 탑이 실제로 어려워져
 *     저급펫 완주율이 25% → 0% 로 떨어졌다. 7.00 에서 재측정:
 *     풀세트 468층·완주 0% / 저급펫 483층·25%→17% / 중급 491층 / 고급 500층·100%.
 *     목표 판정 6종 전부 통과.
 *
 ...
 *
 *   참고로 던전의 6.35 와 직접 비교하면 안 된다. 저쪽은 보스 1 + 호위 6 편성이고
 *   탑은 일반 동굴 적 7기라 같은 배율이라도 체감이 다르다.
 */
export const POWER_MAX = 7.00;
