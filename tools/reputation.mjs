...
function questCountCheck() {
  ...
  /* ★ 기준을 «S랭크로 몇 건» 으로 바꿨다.
   *   만점(300)까지 F랭크로 가는 건 원래 설계된 경로가 아니다 — 저랭크는 초반용이고,
   *   평판을 끝까지 밀어붙이는 건 고랭크를 도는 후반 플레이다.
   *   게다가 감쇠(REP_DECAY_PER_DAY)가 들어간 뒤로 만점은 «도달점» 이 아니라
   *   **유지 상태**다. F랭크 건수로 재는 건 뜻이 없다. */
  const sGain = REP_QUEST_GAIN.S || 1;
  const sMax = Math.ceil((REP_MAX - START_REP) / sGain);
  console.log(`  평판 만점까지 S랭크로 ${sMax}건 (감쇠가 없다고 볼 때)`);
  verdict(sMax <= 40,
    `평판 만점은 S랭크 ${sMax}건 — 장기 목표로 적당하다.`,
    `평판 만점에 S랭크 ${sMax}건이 필요하다 — 사실상 도달 불가다.`);

  verdict(fMax <= 400 && cMax <= 120,
