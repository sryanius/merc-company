...
//   y     = 8  + slot.y * 44
import { clamp, scaleStats } from '../src/core/util.js';
import { RNG } from '../src/core/rng.js';
import { chooseAction, isDamaging, makeBasicSkill } from '../src/battle/ai.js';
...
export function createBattle(cfg = {}) {
  ...
  function rollDamage(src, tgt, power, dmgType) {
    ...
    const _J = Number(process.env.JITTER || '0.07');
    let raw = src.st.atk * power * mit * rng.float(1 - _J, 1 + _J);
  ...
  function tick(dt) {
    ...
    const _ORDER = process.env.TICKORDER === 'enemyfirst'
      ? units.slice().sort((a, b) => (a.side === b.side ? a.idx - b.idx : (a.side === 'enemy' ? -1 : 1)))
      : units;
