...
console.log('\n── 2. 조작을 잡는가');
{
  ...
  const caught = [
    ...
    /* ★ 이 두 줄은 **서로 다른 검사**를 잰다. 하나로 합치면 한쪽이 죽어도 모른다.
     *
     *   원래 한 줄(`renown: 999_999` → 기대 B)뿐이었는데, 나중에 checkStatic 에
     *   «일차 기준 명성 상한» 이 생기면서(21df0cc) 그게 먼저 걸려 A 가 됐다.
     *   기대값을 안 고쳐서 **그때부터 이 도구는 계속 빨간불이었다** — 80여 커밋 동안.
     *   더 강한 등급으로 잡히는 건 좋은 일이므로 기대값을 A 로 고치고,
     *   증가분 경로(B)는 **정적 상한에 안 걸리는 값**으로 따로 잰다. */
    ['명성 폭증 — 정적 상한', prev, base({ day: 101, renown: 999_999 }), 'A'],
    ['명성 폭증 — 증가분 (정적 상한 아래)', prev, base({ day: 101, renown: 100_000 }), 'B'],
  ];
