...
const RAW_SETS = {
  ...
  /* ── 던전4 「성좌의 은총」 — **사제 전용.**
   *
   * ★★ 원래는 전 아키타입이었다. 그런데 풀 효과 `constellation_grace` 는
   *   «쓰러질 때 되살아나고 아군 전체를 회복» 이라 생존 축이고, 사제가 들어야 값어치가 산다.
   *   전 아키타입으로 두면 배분이 전투력 순이라 앞사람들이 조각을 나눠 갖고
   *   **사제가 계속 못 쓴다.** 배점으로 임자를 밀어 봤지만(prefer) 여러 벌이 돌아다니는
   *   실제 세이브에서는 여전히 10·5·5·3·3 으로 흩어졌다.
   *
   *   그래서 제작자 결정: **아예 사제만 입는다.** «나중에 사제 계열 서포터 클래스를
   *   만들든가 하자» — 그때 archs 에 그 계열을 더하면 된다.
   *   `archs` 는 `equipIssue`(gear.js:1237)가 강제하므로 다른 계열은 아예 못 낀다. */
  constellation: {
