...
export const LINEAGE_TRAIT = {
  ...
  swordsman: {
    ...
    riposte: 0.55,       // 받은 피해의 이만큼을 되돌려준다 (근접 피격에 한함)
  },
  ...
  spearman: {
    ...
    intercept: 0.42,
    interceptCut: 0.25,  // 창으로 받아내는 만큼 깎인다 (수호의 0.45 보다 적다)
    /* ★★ **이 둘을 올리지 마라.** 0.62 / 0.35 로 올렸더니 기여도가
     *   +1.2 → **−3.4** 로 떨어졌다 (조합 5개 × 300판). 깎아도 마찬가였다 —
     *   대신 맞는 횟수가 늘면 앞줄이 먼저 무너져 결국 손해다.
     *   «가끔, 위험할 때만» 이 이 특성의 생명선이다. */     // 파고드는 적 근접을 자기가 대신 받는 확률
  },
  ...
  archer: {
    ...
    chargeSlow: 0.32,    // 적 근접의 돌진 시간이 이만큼 늘어난다
  },
  ...
  apprentice: {
    ...
    wardShield: 0.45,    // 최대 HP 의 이만큼을 방패로 들고 시작
    wardRegen: 0.30,     // 초당 방패 최대치의 이만큼 차오른다 (재생)
  },
