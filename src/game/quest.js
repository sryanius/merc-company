 ...
 * 실패 페널티는 성공 보상의 절반(최소 1)이라 실패해도 평판이 순증하는 구간은 없지만,
 * 되돌리는 비용이 얻는 비용의 두 배라서 무리한 고랭크 도전이 그냥 손해가 되지는 않는다.
 */
/* ★ 상한이 300 으로 늘면서 곡선을 위로 기울였다.
 *   예전(F2…S10)은 랭크별 차이가 완만해서 상한 3배를 그대로 3배 시간으로 만들었다.
 *   높은 랭크일수록 훨씬 빨리 쌓이게 하면 «초반엔 더디고 뒤로 갈수록 수월» 이 된다 —
 *   제작자가 도시 개편에서 노린 것과 같은 방향이다. */
export const REP_GAIN = { F: 2, E: 3, D: 5, C: 8, B: 12, A: 18, S: 26 };
/** 실패 시 하락폭 — 성공 보상의 절반, 최소 1 */
const repLoss = (gain) => Math.max(1, Math.round(gain / 2));
