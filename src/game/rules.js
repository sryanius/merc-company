...
import { MAX_LEVEL, HERO_MAX_LEVEL, DAYS_PER_WEEK, DAYS_PER_MONTH, MAX_SQUADS, ROSTER_CAP_MAX } from '../data/limits.js';
...
/**
 * 황금 나락 — 실측 (`node tools/abysspower.mjs --grid=3` · 2026-09-09 · 806구성/합법 539 · 617s)
 *
 * ★ §184 **격자 실측으로 갈아 끼웠다.** §173~§180 은 옛 실측을 ×1.3 으로 옮긴 «잠정 변환값» 이었고,
 *   깊은 쪽이 크게 어긋나 있었다 — 잠정 [100k, 394] 인데 실측은 **240**, [190,470, 500] 인데 실측 **408**.
 *   즉 잠정표는 깊은 쪽에서 너무 헐거웠다 (오탐이 아니라 **놓침** 쪽으로).
 * ★ 이번 격자부터 **세트 2·3단**과 **각성 영웅**을 축에 넣었다 (그 둘이 깊은 쪽 포락선을 정한다).
 *   실측 포락선 주인: 100k=Lv30 S 2단 · 150k=Lv50 영웅 2단 · 190k=Lv60 영웅 2단 · 250k=Lv100 영웅 2단.
 * ★ 심층별 최소 전력: 96=50,514 · 200=90,545 · 300=141,443 · 400=169,558 · 500=235,125.
 *   ⇒ 전력 27,127 로는 71심층이 한계다 (그 치트가 신고한 96 은 필요 전력의 54%).
 * ★ 400 위는 각성 영웅이 있어야 열린다(§177 관문) — 표의 400+ 행은 전부 영웅 편성이다.
 *   옛 표: [5k,18] [10k,29] [20k,51] [30k,66] [50k,86] [75k,108] [100k,128] [190,470,163] (§104, 상한 130 격자)
 */
export const ABYSS_POWER_CURVE = [
  [5_000, 20], [10_000, 36], [20_000, 55], [30_000, 71],
  [50_000, 92], [75_000, 150], [100_000, 240], [150_000, 349],
  [190_470, 408], [250_000, 500],
];
...
function absoluteOddities(s) {
  ...
  /* ★ §173 — 500심층·소탕과 함께 12 → 30 으로. 소탕은 매주 «기록 + 1» 에서 만피로 서게 하므로 강해진
   *   부대는 한 주에 100심층 넘게 뛸 수 있다 (실측: 1단 → 2단 풀세트가 벽 100 → 200). 20주면 600 이라
   *   바닥(500)이 그 안에 들어온다 — 영웅·3단 세트를 20주 안에 다 갖추는 계정은 없다. */
  if (s.abyssBest > abyssRuns * 30) bad.push(`나락 ${s.abyssBest}심층 · ${s.day}일차면 입장 ${abyssRuns}회`);
