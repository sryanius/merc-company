...
import { MAX_LEVEL } from '../game/merc.js';
import { DAYS_PER_WEEK, DAYS_PER_MONTH } from '../game/state.js';
...
export function checkStatic(s) {
  ...
  if (s.squadsN < 0 || s.squadsN > 5) bad.push(`부대 ${s.squadsN}`);
  if (s.rosterN < 0 || s.rosterN > 70) bad.push(`단원 ${s.rosterN}`);
