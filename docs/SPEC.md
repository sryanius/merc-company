...
### 9.1 무한의 탑 — `data/tower.js` · `game/tower.js`

| 규칙 | 값 |
|---|---|
| 층수 | 1 ~ 500 |
| 입장 | **매달 1일에만.** 그날 안에 끝까지 오른다 (날짜 안 넘어감, 부대 안 잠김) |
| 층 비용 | 층수 × `GOLD_PER_FLOOR(2)` — 1~500층 누적 **250,500G** |
| 소탕 | 다음 달 재입장 시 `최고 기록 − SWEEP_BACKOFF(100)` 층까지 전투 없이 통과. **골드는 낸다** |
| 체력 | **층을 넘어도 안 채워진다.** `REST_EVERY(25)` 층마다 전원 회복 |
| 쓰러진 단원 | 다음 회복 지점까지 편성에서 빠진다 |
| 보상 | 펫. `dropChance` = 2.0%(1층) → 5.5%(500층). 완주 1회 기대 **약 19마리** |

**입장일 판정은 `week === 1 && dayOfWeek === 1` 이다.** 달력에 `dayOfMonth` 가 없어서
`dayOfWeek === 1` 만 보면 **한 달에 4번(1·8·15·22일) 참**이 된다.
그리고 `advanceDays` 에는 월 경계 훅이 없으므로 "이번 달 다녀옴"을 **플래그로 저장하면 안 된다**
(리셋할 자리가 없다). `tower.lastRunDay` 를 저장하고 날짜로 판정한다.

**진행도는 `state.tower` 에 둔다. `state.dungeons` 에 얹으면 안 된다** —
`normalizeDungeons` 가 항목을 `{bestWave, clearedAt}` 두 키로 재구성해 나머지를 조용히 버린다.

#### 난이도 — 축이 둘이다

`power` 하나로는 500층을 못 나눈다. 실측으로 그 축의 해상도는
`4.70→100% / 5.10→60% / 5.20→36% / 5.90→0%` 이라 의미 있는 밴드가 Δ0.35 뿐이다.
그래서 **체력 이월**을 두 번째 축으로 쓴다 — 한 판 승률이 아니라 연속 생존이 난이도를 만든다.

> 실측: 풀세트 부대가 400층을 만피로는 **100%** 이기지만, 실제 등반은 **378층**에서 멈춘다.

`POWER_MIN 2.70 → POWER_MAX 6.20`, 지수 `POWER_CURVE 1.85`.
보조로 층당 적 수가 3 → 7 로 는다(`floorEnemyCount`).

**던전의 5.36 과 직접 비교하면 안 된다.** 저쪽은 보스 1 + 호위 6 편성이고 탑은 일반 적 7기다.

#### 전투 화면을 안 쓴다

`ui/battle.js` 에는 자동 진행 경로가 **의도적으로 없다**(§5차 세션 계약).
500층을 그쪽으로 돌리면 최소 500회 클릭이고, `fastForward()` 의 **12웨이브 하드 캡** 때문에
13층에서 런 전체가 조용히 패배 처리된다.
→ 등반은 `game/tower.js` 의 헤드리스 `simulate()` 루프로 돌리고 층별 결과를 로그로 보여 준다.

#### 난수

층마다 독립 RNG: `hashStr('tw#층#부대#날짜') ^ state.seed`.
**드랍에 `globalRng` 를 쓰면 안 된다** — `load()` 가 그 시드를 게임 최초 상태로 되감아
새로고침으로 원하는 펫이 나올 때까지 리롤할 수 있게 된다.

### 9.2 펫 — `data/pets.js` · `game/pet.js`

인스턴스는 `{ uid, sid, grade, hp }` 만 저장한다. 나머지는 종 정의에서 파생한다.
`uid` 는 `state.petSeq` 로 채번한다 — `core/rng.js` 의 `uid()` 는 `Math.random` 을 써서
전투 결과 키(`damageDealt`/`kills`)의 재현이 깨진다.

최종 스탯 = `base × PET_TIER_MULT[tier-1] × PET_GRADE_MULT[grade]`.
속도·치명·회피는 배율을 **반만** 먹인다(그대로 곱하면 상위 펫이 행동을 독점한다).

| 역할 | 구현 자리 | 엔진 수정 |
|---|---|---|
| `attacker` | 그냥 유닛으로 세운다 | 없음 |
| `healer` | 기존 `heal` 효과 스킬 (`pet_mend` / `pet_bless`) | 없음 |
| `buffer` | **UnitDef 를 만들 때** 단원 스탯에 곱한다 (`quest.js applyPetBuff`) | 없음 |
| `guardian` | `applyDamage` 재대상 | **있음** |

`buffer` 를 전투 전에 적용하는 이유: 엔진 버프 대상(`ST_KEYS`)에 `hp` 가 없어
**전투 중에는 최대 체력을 못 올린다.** 전투 전에 곱하면 `maxHp` 까지 오른다.

#### 진형에 넣지 않는다

`quest.js slotsOf()` 는 `f.slots.length === 7` 이 아니면 진형을 통째로 `FALLBACK_SLOTS` 로 갈아 버린다.
진형을 10칸으로 늘리는 순간 **플레이어가 골드로 산 진형 12개가 전부 기본 좌표·무보정이 된다**
(예외도 로그도 없다). 그래서 펫은 `PET_SLOTS`(x=0.96 고정 열)에 세우고
`slotIndex` 를 **넘기지 않는다** — `formationMods` 는 범위 밖 인덱스를 `slots[0]`(대개 전열) 으로
폴백시켜 펫에게 엉뚱한 전열 보정을 건다.

### 9.3 엔진 변경 — `battle/engine.js`

1. **`hasGuardians` 게이트.** 전투 rng 는 단일 스트림이라 난수를 한 번만 더 굴려도
   그 뒤의 치명타·회피가 전부 밀린다. 수호 펫이 실제로 있을 때만 굴린다.
   → 펫 없는 전투는 **바이트 단위로 동일**하다 (랭크 승률 F100/E74.1/D63.0/C62.2/B60.7/A57.4/S52.6 불변 실측).
2. **`applyDamage` 재대상.** 원래 이 엔진에는 대상을 바꾸는 훅이 없었다(가로챌 수 있는 건 죽음뿐).
   규약: 펫이 맞는 건 안 넘김(연쇄 방지) / dot 은 안 넘김 / 피해량은 **원래 대상 기준으로 이미 계산된 값**이라
   대신 맞는 쪽이 자기 방어력으로 재계산하지 않고 `guardCut` 으로만 깎는다.
3. **승패에서 펫 제외.** `aliveFighters()` 를 따로 둔다 — `aliveAllies()` 는 **표적 선택**에도 쓰여서
   거기서 펫을 빼면 회복이 펫을 못 살리고 적도 펫을 못 때린다.
   `hpRatioOf`(시간초과 판정)와 MVP 후보에서도 뺀다.

### 9.4 계측

`node tools/tower.mjs` — 층별 만피 승률표 + **등반 도달 층**(진짜 지표) + 목표 판정 6종.
아군 편성을 자체 조립하지 않고 실제 `state` 를 만들어 `questBattleDefs` 를 태운다.
